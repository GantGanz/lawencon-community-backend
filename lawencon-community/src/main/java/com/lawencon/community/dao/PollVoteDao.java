...
public class PollVoteDao extends AbstractJpaDao {
	public Long countPollVote(final String pollId) {
		...
		.append("INNER JOIN t_poll_vote v ON v.poll_option_id = o.id ")
		.append("WHERE p.poll_id = :pollId ");
	...
	public Long countPollVoteByPollOption(final String pollOptionId) {
		...
		.append("INNER JOIN t_poll_option o ON o.poll_id = p.id ")
		.append("INNER JOIN t_poll_vote v ON v.poll_option_id = o.id ")
		.append("WHERE o.id = :pollOptionId ");

		Long total = null;
		try {
			final Object userObj = createNativeQuery(str.toString()).setParameter("pollOptionId", pollOptionId).getSingleResult();
			if (userObj != null) {
				total = Long.valueOf(userObj.toString());
			}
		} catch (final Exception e) {
	...
	public Long isVoted(final String pollId, final String userId) {
		...
		.append("INNER JOIN t_poll_option o ON o.poll_id = p.id ")
		.append("INNER JOIN t_poll_vote v ON v.poll_option_id = o.id ")
		.append("WHERE p.id = :pollId ").append(" AND v.created_by = :userId ");
